function [features, labels, keys, T_matrix] = getSPixelFeatures(imsegs)
%      01 - 03: rgb values
...
features = cell(numel(imsegs), 1);  %feature for each superpixel
labels = cell(numel(imsegs), 1);
keys = zeros(715000,2); %maps a feature to image and superpixel

T_matrix = zeros(320000,8); %making the k-1 row matrix
...
for f = 1:numel(imsegs)
    ...
    for i = 1:numspix
      ...
      %Assigning 1 to the feature which is represented 
      if(imsegs(f).super_pixel_labels(i) > 0)
            T_matrix(q,imsegs(f).super_pixel_labels(i)) = 1;
      else
            T_matrix(q,1) = 1;
      end
      q = q+1;
